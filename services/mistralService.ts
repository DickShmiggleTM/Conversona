...
// A common prompt instruction for models to produce structured JSON output.
...
// Fix: The original formatMistralMessages function was logically flawed and contained a recursive call with out-of-scope variables.
// It has been replaced with a correct implementation that follows the standard Mistral API format,
// creating a system message followed by the history and the user's prompt. This is more robust and aligns
// with the approach used in other services like openrouterService.ts.
const formatMistralMessages = (systemPrompt: string, history: ChatMessage[], prompt: string) => {
...
export const getMistralResponse = async (
    ...
    try {
        const response = await fetch(MISTRAL_API_URL, {
            ...
            body: JSON.stringify({
                model: persona.model,
                messages: formatMistralMessages(getSystemInstruction(persona, context, history), history, prompt),
        ...
        if (!response.ok) {
            ...
            throw new Error(`Mistral API request failed: ${errorBody.message}`);
        }
        ...
        if (typeof parsedContent !== 'object' || parsedContent === null) {
            return {
                text: `Error: Model returned non-object JSON response. Content: ${contentString}`,
        ...
        return {
            text: `An error occurred while fetching the response from Mistral. Details: ${errorMessage}`,
