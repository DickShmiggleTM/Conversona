...
Your entire output must be a single, valid JSON object with three keys: "internalMonologue", "responseText", and "sentiment".
...
Do not include any other text, formatting, or code fences outside of this JSON object.
`;
...
const formatMessages = (systemPrompt: string, history: ChatMessage[], prompt: string) => {
    const messages = [
    ...
    try {
        const response = await fetch(OPENROUTER_API_URL, {
            ...
            headers: {
                ...
                'X-Title': 'Persona Duel', // Recommended by OpenRouter
            },
            body: JSON.stringify({
                model: persona.model, // e.g., "google/gemini-flash-1.5"
                messages: formatMessages(getSystemInstruction(persona, context, history), history, prompt),
        ...
        try {
            // Clean the string in case the model wraps the JSON in a markdown code block.
            const cleanedString = contentString.replace(/^```json\s*|\s*```$/g, '');
            const parsedContent = JSON.parse(cleanedString);
             if (typeof parsedContent !== 'object' || parsedContent === null) {
                return {
                    text: `Error: Model returned non-object JSON response. Content: ${cleanedString}`,
            ...
            return {
             ...
             console.warn("Failed to parse JSON from OpenRouter, treating as plain text:", contentString);
             return {
                text: contentString,
        ...
        return {
            text: `An error occurred while fetching the response from OpenRouter. Details: ${errorMessage}`,
